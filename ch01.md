...
## 1.1 Self-Organizing Nervous Systems in Swarms Of Embedded Multitasking Realtime Operating Systems

### 1.1.1 About the FreeRTOS Kernel and CloudKernelOS

CloudKernelOS builds on or extends the FreeRTOS kernel, which is a real-time operating system (RTOS) kernel for embedded devices that has been ported to 40 microcontroller platforms. The FreeRTOS kernel is a collection of C libraries comprised of a real-time kernel and a set of modular libraries that implement complementary functionality. 

CloudKernelOS is a collection of C and Rust libraries which supports shared data between nodes for observability coordination in [Self-Organizing Nervous Systems (SoNS)](https://arxiv.org/abs/2401.13103) for building *swarms that learn.* CloudKernelOS is fundamentally about swarm robotics and highly-distributed observability engineering for intelligent systems of independent, but self-organizing, loosely-coordinated multi-drone swarm technologies. As with FreeRTOS, CloudKernelOS is distributed under the MIT License.
...
Here is the list of repositories where you can submit feature requests:
